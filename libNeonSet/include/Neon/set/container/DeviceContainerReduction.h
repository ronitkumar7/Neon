...
template <typename ReduceOp,
          typename T,
          typename DataIteratorContainerT,
          typename UserComputeLambdaT>
struct DeviceContainerReduction : ContainerAPI
{
   public:
    virtual ~DeviceContainerReduction() override = default;

    DeviceContainerReduction(const std::string&                            name,
                     ...
                     Neon::DataView dataView = Neon::DataView::STANDARD) -> void override
    {
        ...
        if (ContainerExecutionType::device == this->getContainerExecutionType()) {
            bk.devSet().template launchLambdaOnSpanReduction<ReduceOp, T, DataIteratorContainerT, UserComputeLambdaT>(
                mExecution,
                kernelConfig,
